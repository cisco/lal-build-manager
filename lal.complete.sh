...
_lal()
{
    ...
    local -r subcommands="build clean configure export fetch help init
                list-components remove shell stash status update upgrade verify"

    local has_sub
    for (( i=0; i < ${#words[@]}-1; i++ )); do
        if [[ ${words[i]} == @(build|clean|configure|export|fetch|help|init|list-components|remove|shell|stash|status|update|upgrade|verify) ]]; then
            has_sub=1
        fi
    done
    ...
    local special i
    for (( i=0; i < ${#words[@]}-1; i++ )); do
        if [[ ${words[i]} == @(build|remove|update) ]]; then
            special=${words[i]}
        fi
    done
