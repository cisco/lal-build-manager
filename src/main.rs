...
fn handle_manifest_agnostic_cmds(
    ...
    let res = if let Some(a) = args.subcommand_matches("export") {
        let env = if let Some(override_env) = a.value_of("environment") {
            Some(override_env)
        } else {
            explicit_env
        };
...
fn handle_environment_agnostic_cmds(args: &ArgMatches, mf: &mut Manifest, backend: &dyn Backend) {
    let res = if let Some(a) = args.subcommand_matches("status") {
        ...
        .subcommand_matches("list-supported-environments")
        .is_some()
    {
        ...
        lal::propagate::print(
            mf,
            &a.values_of("component")
                .unwrap()
                .map(ToString::to_string)
                .collect::<Vec<_>>(),
            a.is_present("json"),
        )
    } else if let Some(a) = args.subcommand_matches("channel") {
        handle_channel_cmds(&a, mf)
    } else {
        return;
    };
    result_exit(args.subcommand_name().unwrap(), res);
...
fn handle_channel_cmds(args: &ArgMatches, mf: &mut Manifest) -> LalResult<()> {
    let current_channel = lal::channel::get(mf);
    let mut is_testing = false;

    let new_channel = {
        if let Some(a) = args.subcommand_matches("set") {
            a.value_of("channel").unwrap().to_owned()
        } else if args.subcommand_matches("reset").is_some() {
            "/".to_owned()
        } else if args.subcommand_matches("testing").is_some() {
            is_testing = true;
            format!("{}/{}", current_channel, Channel::TESTING)
        } else if let Some(a) = args.subcommand_matches("append") {
            let subchannel = a.value_of("subchannel").unwrap();
            format!("{}/{}", current_channel, subchannel)
        } else {
    ...
    lal::channel::set(mf, &new_channel, is_testing)
}
...
fn handle_env_command(
    ...
    if let Some(a) = args.subcommand_matches("env") {
        if a.subcommand_matches("update").is_some() {
            ...
            // reset will fail.. possible to fix, but complicates this file too much
...
#[cfg(feature = "upgrade")]
fn handle_upgrade(args: &ArgMatches<'_>, cfg: &Config) {
...
fn handle_docker_cmds(
    ...
    let res = if let Some(a) = args.subcommand_matches("verify") {
        ...
        let mut flags = lal::verify::Flags::default();
        if a.is_present("simple") {
            flags |= lal::verify::Flags::SIMPLE;
        }
        if a.is_present("testing") {
            flags |= lal::verify::Flags::TESTING;
        }
        lal::verify::verify(mf, env, flags)
    } else if let Some(a) = args.subcommand_matches("build") {
        let mut flags = lal::verify::Flags::default();
        if a.is_present("simple-verify") {
            flags |= lal::verify::Flags::SIMPLE;
        }
        if a.is_present("testing") {
            flags |= lal::verify::Flags::TESTING;
        ...
        let modes = ShellModes {
            ...
            capabilities: values_t!(a.values_of("cap-add"), String).unwrap_or_else(|_| vec![]),
        };
        lal::build(cfg, mf, &bopts, env, modes)
...
fn main() {
    let mut app = App::new("lal")
        ...
        .arg(
            Arg::with_name("environment")
                ...
                .takes_value(true)
                .hidden(true)
                .help("DEPRECATED - Provide a default environment for commands which do not use the manifest's environment"),
        )
        .arg(
        ...
        .subcommand(
            SubCommand::with_name("build")
                ...
                .arg(
                    Arg::with_name("component").help(
                        "Build a specific component (if other than the main manifest component)",
                    ),
                )
                .arg(
                ...
                .arg(
                    Arg::with_name("testing")
                        ...
                        .long("testing")
                        .hidden(true)
                        .help("Allows testing components in the channel"),
                )
                .arg(
                    Arg::with_name("force")
                        .long("force")
            ...
            SubCommand::with_name("export")
                ...
                .arg(
                    Arg::with_name("environment")
                        ...
                        .short("e")
                        .takes_value(true)
                        .help("The environment to query. If this is not configured then a global environment argument must have been passed")
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("env")
                ...
                .subcommand(
                    SubCommand::with_name("reset").about("Return to the default environment"),
                ),
        )
        .subcommand(
            SubCommand::with_name("stash")
            ...
            SubCommand::with_name("query")
                ...
                .arg(
                    Arg::with_name("channel")
                        ...
                        .short("c")
                        .takes_value(true)
                        .help("The channel to query. If not specified then the default channel is used")
                )
                .arg(
                    Arg::with_name("component")
                        .required(true)
        ...
        .subcommand(
            SubCommand::with_name("update-all")
        ...
        .subcommand(
            SubCommand::with_name("list-supported-environments")
                .setting(AppSettings::Hidden)
                .about("list supported environments from the manifest"),
        )
        .subcommand(
            SubCommand::with_name("list-environments")
                .setting(AppSettings::Hidden)
                .about("list environments that can be used with lal build"),
        )
        .subcommand(
            SubCommand::with_name("list-configurations")
                .setting(AppSettings::Hidden)
                .arg(
                    Arg::with_name("component")
                        .required(true)
        ...
        .subcommand(
            SubCommand::with_name("channel")
                .about("Manages channel configurations")
                .subcommand(
                    SubCommand::with_name("set")
                        .about("Set a specific channel coordinate in the manifest file")
                        .arg(
                            Arg::with_name("channel")
                                .required(true)
                ...
                .subcommand(
                    SubCommand::with_name("reset")
                        .about("Return to using the default channel in the manifest file"),
                )
                .subcommand(
                    SubCommand::with_name("append")
                        .about("Append a subchannel to the current channel")
                        .arg(
                            Arg::with_name("subchannel")
                                .required(true)
                                .help("The subchannel to append"),
                        ),
                )
                .subcommand(
                    SubCommand::with_name("testing")
                        .setting(AppSettings::Hidden)
                        .about("Creates a testing subchannel - for use in tooling only"),
                ),
        );

    if cfg!(feature = "upgrade") {
        app = app.subcommand(
            SubCommand::with_name("upgrade").about("Attempts to upgrade lal from artifactory"),
        );
    }
    ...
    handle_environment_agnostic_cmds(&args, &mut manifest, backend.deref());
        ...
        .any(|e| e == env)
    {
        ...
        debug!("Running {} command in supported {} environment", sub, env);
    } else {
        let sub = args.subcommand_name().unwrap();
